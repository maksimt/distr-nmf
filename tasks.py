...
from config import base_path, log_filename, available_RAM, remove_intermediate
...
logging.basicConfig(
    filename=os.path.join(base_path, log_filename),
...
class GenDataset(produce_autotargeting_class(
    ...
    def run(self):
        ds = datasets.load_dataset(self.dataset_params['dataset_name'])
        n, d = ds['X'].shape
        X_I = ds['X'][range(self.group_id, n, self.n_groups), :].toarray()
